...
<head>
    ...
    <title>UMKM - SIP PESANGGRAHAN</title>
    ...
    <style>
        ...
        .page-header {
            background: linear-gradient(rgba(0, 51, 102, 0.8), rgba(0, 51, 102, 0.8)),
                        url('https://images.unsplash.com/photo-1555529669-e69e730f162b?q=80&w=2070&auto=format&fit=crop');
...
<body class="bg-light">
    ...
    <div class="page-header shadow-sm">
        <div class="container">
            <h1 class="display-5 fw-bold mb-3">UMKM Warga</h1>
            <p class="lead mb-0 opacity-75">Katalog produk unggulan dan usaha kreatif lokal masyarakat Kelurahan Pesanggrahan</p>
        </div>
    </div>
    ...
    <div class="container py-5 mb-5 flex-grow-1">
        <div class="row g-4 justify-content-center">
            ...
            <div class="col-md-4">
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100 bg-white">
                    <div style="height: 200px; background: #e9ecef; position: relative;">
                        @if($u->foto)
                            <img src="{{ asset('storage/' . $u->foto) }}" class="w-100 h-100" style="object-fit: cover;" alt="{{ $u->nama_usaha }}">
                        @else
                            <div class="d-flex justify-content-center align-items-center h-100">
                                <i class="bi bi-shop text-secondary opacity-50" style="font-size: 4rem;"></i>
                            </div>
                        @endif
                        <span class="badge position-absolute rounded-pill px-3 py-2 text-dark fw-bold shadow-sm" style="background-color: #ffcc00; top: 15px; right: 15px; font-size: 0.75rem;">{{ $u->kategori }}</span>
                    </div>
                    <div class="card-body p-4">
                        <h5 class="fw-bold text-primary mb-1">{{ $u->nama_usaha }}</h5>
                        <p class="small text-muted mb-3"><i class="bi bi-person me-1"></i>Pemilik: {{ $u->pemilik }}</p>
                        <p class="small text-secondary mb-4" style="text-align: justify;">{{ $u->deskripsi }}</p>
                        @if($u->kontak)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $u->kontak) }}" target="_blank" class="btn btn-sm btn-success rounded-pill px-3 fw-bold small"><i class="bi bi-whatsapp me-1"></i> Hubungi Toko</a>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 text-center py-5">
                ...
                <p class="text-muted mt-3">Data usaha lokal belum ditambahkan oleh Admin.</p>
            </div>
            @endforelse
        </div>
    </div>
